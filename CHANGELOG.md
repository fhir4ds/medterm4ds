...
- Added `scripts/build_embedding_index.py` to produce `reports/fhir4px/embedding_index.jsonl` from `canonical_codes.csv`. Each canonical code becomes one JSON record with 4 vector texts (technical, synonyms, friendly, hierarchy) plus metadata (semantic_types, ATC levels for medications, candidate_count, rule). Synonyms capped at K=8 per code, prioritized by source (MSH > MEDLINEPLUS > CHV > SNOMEDCT_US > ICD10CM > RXNORM > LNC). Hierarchy is source-specific 3-level ancestor chain. Output is 134 MB across 196,509 records.
