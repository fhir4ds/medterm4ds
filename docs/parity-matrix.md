...
| SNOMED strategy | `LocalDuckDBEngine._resolve_snomed` | Target-first routing priority `ICD10CM > ICD10PCS > LNC > CPT > HCPCS`, with each target hierarchy walked before guarded direct SNOMED fallback; SNOMED does not route through RXNORM for patient-friendly naming | SNOMED top-level guard is broader and intentional; structured provenance is added; original display uses the SNOMED preferred term while preserving the fully specified name as `technical_name` | Source-by-source real-data matrix needs regeneration after the target-first/no-RXNORM policy change |
