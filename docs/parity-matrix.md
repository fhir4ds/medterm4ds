...
| `drugs_for_indication` | `drugs_for_indication`, `indication_search` | Indication workflow | medterm4ds now resolves UMLS Metathesaurus relationship-backed medications (may_treat / may_prevent / may_diagnose / contraindicated_with_disease) from a condition or explicit source+code; response shape is the new uniform contract (no `drug_name_context`, `reason`, or `terminology_context_only` status) | Unit coverage exists |
